...
module.exports.getFileHandles = function (request) {
  if (!this.exists(request.files)) return [];
...
module.exports.cntUploadedFiles = function (request) {
  if (!this.exists(request.files)) return 0;

  var cnt = 0;
  Object.keys(request.files).forEach(function (item) {
    if (Array.isArray(request.files[item])) cnt += request.files[item].length;
    else cnt++;
  });
...
module.exports.existsFileHandle = function (request, handleName) {
  if (!this.exists(request.files)) return false;

  if (
    request.files[handleName] === null ||
    request.files[handleName] === undefined
  )
    return false;
...
module.exports.getUploadedFilesAsArray = function (
  request,
  handleName,
  expanded
) {
  if (!this.existsFileHandle(request, handleName)) return [];
...
module.exports.isFileObject = function (obj) {
  ...
  return (
    this.exists(obj.name) && this.exists(obj.size) && this.exists(obj.mimetype)
  );
};
...
module.exports.isMimeTypePicture = function (mimeType) {
  if (!this.exists(mimeType)) return false;
...
module.exports.isFileObjectPicture = function (obj) {
  if (!this.isFileObject(obj)) return false;
...
module.exports.createRandomFilename = function (fileNameLength, extension) {
  if (!this.exists(fileNameLength)) fileNameLength = 4;

  if (fileNameLength < 4) fileNameLength = 4;
  ...
  if (this.exists(extension)) randomName += "." + extension;
