...
module.exports.parseGermanDateTimeString = function (input) {
  ...
  else datetimeobj = DateTime.fromFormat(input, "dd.MM.yyyy");
...
module.exports.formatToGermanDate = function (datetimeobj) {
  if (!this.isDateTime(datetimeobj)) return null;
...
module.exports.modifyDateTime = function (
  datetimeobj = null,
  y = 0,
  m = 0,
  d = 0,
  ...
  if (!this.isDateTime(datetimeobj)) datetimeobj = this.getNow();
  ...
  if (y != 0) options.years = y;
  if (m != 0) options.months = m;
  if (d != 0) options.days = d;
  if (h != 0) options.hours = h;
  if (i != 0) options.minutes = i;
  if (s != 0) options.seconds = s;
...
module.exports.strStartsWith = function (haystack, needle) {
  if (this.isNull(haystack) || this.isUndefined(haystack)) return false;

  if (this.isNull(needle) || this.isUndefined(needle)) return false;
